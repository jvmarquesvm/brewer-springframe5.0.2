...
public class VendaRepositoryImpl implements VendaRepositoryQueries {
	...
	private void adicionarFiltro(VendaFilter filtro, Criteria criteria) {
		...
		if (filtro != null) {
			...
			if (!StringUtils.isEmpty(filtro.getCpfOuCnpjCliente())) {
				criteria.add(Restrictions.eq("c.cpfOuCnpj", TipoPessoa.removerMascara(filtro.getCpfOuCnpjCliente())));
			}
		}
	}
	...
	public List<VendaMes> totalPorMes() {
		...
		for(int i = 1; i <= 12; i++) {
			...
			boolean possuiMes = vendaMes.stream().filter(v -> v.getMes().equals(mes)).findAny().isPresent();
			if(!possuiMes) {
				vendaMes.add(i -1, new VendaMes(mes, 0));
			}
